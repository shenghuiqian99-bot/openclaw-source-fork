...
- Agents/instructions: add Claude-compatible workspace instruction loading for `CLAUDE.md`, `.claude/CLAUDE.md`, and `CLAUDE.local.md` alongside `AGENTS.md`, auto-load `.claude/rules/**/*.md` as modular instruction files, carry those files through bootstrap hooks, subagent/cron allowlists, post-compaction refresh, and safeguard compaction context extraction, strip rule-file frontmatter before injection, and support workspace-scoped `@path` instruction imports with cycle/depth protection plus duplicate-root suppression.
- Agents/context: add instruction-load diagnostics to `systemPromptReport` and `/context detail|json`, including instruction kind, load mode, evaluation order, rule frontmatter stripping, and `@path` import error counts for the active workspace instruction set.
- Agents/instructions: add opt-in path-scoped `.claude/rules/**/*.md` routing via `ruleContextPaths`, matching YAML frontmatter `paths` globs only when callers supply explicit workspace-relative route context, bypassing session bootstrap cache for routed runs, exposing matched rule patterns/paths in instruction diagnostics, and exposing a repeatable `openclaw agent --rule-path <path>` CLI entrypoint for both gateway and embedded agent runs.
- Agents/context: add persisted instruction-load diagnostics aggregation surfaces in `openclaw status --deep`, `status --json --deep|--all`, and a read-only `openclaw doctor --deep` contribution, summarizing the latest known `systemPromptReport.instructionFiles` per agent session without triggering new agent runs.
- Control UI/debug: render persisted instruction-load diagnostics as a dedicated debug-panel section, prefer the dedicated `instructions.diagnostics` RPC with compatibility fallback, add Debug-page filters for `agentId`, `sessionKey`, and `workspaceDir`, let operators expand each report to inspect the full persisted entry list and related session/workspace metadata, add one-click quick-filter actions that jump directly to the current agent, session, workspace, or exact persisted report, expose copy actions for the active diagnostics query and CLI command plus per-report params, query object, reproducible CLI command, current report, and combined debug bundle, allow the current diagnostics query to be written to or invoked from the Manual RPC panel without leaving the browser, and let each report either stage or directly invoke a report-scoped Manual RPC diagnostics query for precise raw-result inspection.
- Gateway/debug: add a dedicated read-only `instructions.diagnostics` operator-read RPC for the latest persisted instruction-load reports per agent, extend it with validated optional `agentId`, `sessionKey`, and `workspaceDir` filters that apply before latest-match selection, and switch the Control UI debug panel to consume it with a fallback to the `status` payload for compatibility.
